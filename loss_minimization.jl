...
Γ_stabilization = stabilization_level * Matrix(I, dim_output, dim_output)
    #Q this is used like the noise is used in sinusoidal.jl. This is built here bc the model does not include noise?

# The function is positive so to minimize it we may set the target to be 0,
G_target = [0]  # analogous to observed
nothing # hide
...
initial_ensemble = EKP.construct_initial_ensemble(rng, prior, N_ensemble)
    #Q this is just sampling N_sample samples yes? nothing funky happening in construct_initial_ensemble?
...
# with `Inversion()`). 
#Q what other process types do we have
